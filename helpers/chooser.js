...
const addToWinLose = (superiorId, inferiorId) => {
    lookupTable.forEach((v, k) => {
        if (
            v.id === superiorId &&
            lookupTable[k]["versusLog"].indexOf(inferiorId) === -1
        ) {
        ...
        if (
            v.id === inferiorId &&
            lookupTable[k]["versusLog"].indexOf(superiorId) === -1
        ) {
...
const processAnswers = answerArrayOfKeys => {
...
// Simple helper
const howManyLeft = () => lookupTable.filter(v => v.beaten < TOP).length;
